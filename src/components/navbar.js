    ...
    return (
        <div>
            <header class="flex backdrop-blur-lg bg-transparent shadow-md sm:justify-between sm:flex-nowrap z-50 w-full bg-white text-sm py-4 px-[9.5rem] dark:bg-gray-800 ">
                ...
                <div className='sm:hidden absolute right-6 top-[1.8rem] cursor-pointer' onClick={() => setOpen(!open)}>{open ? <IoMdClose size={35} /> : <RxHamburgerMenu size={35} />}  </div>
                <nav className='flex sm:hidden'>

                    <div className={`flex flex-col gap-8 transition-all ease-in z-0 pt-4 ${open ? 'mt-[5rem]' : ' mt-[-100rem]'} `}>
